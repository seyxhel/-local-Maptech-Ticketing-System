...
export default function AdminOnHoldTickets() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // On-hold means call+priority workflow is not completed yet.
        const onHoldTickets = raw
          .filter((ticket: BackendTicket) => !ticket.confirmed_by_admin || !String(ticket.priority || '').trim())
          .sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime());
        if (cancelled) return;
        setTickets(onHoldTickets.map(mapBackendTicketToUI));
      } catch {
        if (!cancelled) toast.error('Failed to load on-hold tickets.');
      } finally {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-3 text-gray-500">Loading on-hold tickets...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">On Hold Tickets</h1>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[800px] text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {paged.length === 0 && (
                <tr><td colSpan={8} className="px-6 py-12 text-center text-gray-400 dark:text-gray-500">No on-hold tickets match your filters.</td></tr>
              )}
              {paged.map((ticket) => (
        ...
        <div className="fixed inset-0 z-[9999] bg-black/50 flex items-center justify-center p-4" onClick={() => setShowFilter(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">Filter On Hold Tickets</h3>
