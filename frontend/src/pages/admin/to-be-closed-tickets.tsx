...
export default function AdminToBeClosedTickets() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const toBeClosedTickets = raw
          .filter((ticket: BackendTicket) => ticket.status === 'pending_closure')
          .sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime());
        if (cancelled) return;
        setTickets(
          toBeClosedTickets.map((ticket) => ({
            ...mapBackendTicketToUI(ticket),
            status: 'Pending Closure',
          })),
        );
      } catch {
        if (!cancelled) toast.error('Failed to load to-be-closed tickets.');
      } finally {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-3 text-gray-500">Loading to-be-closed tickets...</span>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">For Closing Tickets</h1>
        </div>
        <p className="text-gray-500 dark:text-gray-400">Tickets that are ready for admin closure review</p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[800px] text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {paged.length === 0 && (
                <tr><td colSpan={8} className="px-6 py-12 text-center text-gray-400 dark:text-gray-500">No to-be-closed tickets match your filters.</td></tr>
              )}
              {paged.map((ticket) => (
        ...
        <div className="fixed inset-0 z-[9999] bg-black/50 flex items-center justify-center p-4" onClick={() => setShowFilter(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">Filter To Be Closed Tickets</h3>
